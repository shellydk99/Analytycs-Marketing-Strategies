...
WHERE ride_id = ''
   ...
   OR member_casual = '';
```
1.298.358 rows affected

## c) Checking for duplicate
```
...
## d) Check ride_id length (ride_id should have length of 16) 
```
...
## e) Check rideable_type (must have 3 different type)
```
...
## f) Check member casual (must have 2 different type)
```
...
```
 ...
 ## g) Check duration more than 24 hours
```
...
```
```
DELETE FROM db_cyclistic..tripdata_2022						--- 157 rows affected
WHERE DATEDIFF(MINUTE, started_at, ended_at) >= 1440;
```

## h) Check duration less than 1 minute
```
SELECT started_at, ended_at
FROM db_cyclistic..tripdata_2022
...
```
```
DELETE FROM db_cyclistic..tripdata_2022						--- 105909 rows affected
WHERE DATEDIFF(MINUTE, started_at, ended_at) <= 1;
```
